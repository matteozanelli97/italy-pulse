...
import { ITALIAN_CITIES } from '@/lib/constants';
...
export async function GET() {
  try {
    ...
    const lats = ITALIAN_CITIES.slice(0, 10).map((c) => c.lat).join(',');
    const lngs = ITALIAN_CITIES.slice(0, 10).map((c) => c.lng).join(',');
    ...
    const stations: AirQualityStation[] = (Array.isArray(data) ? data : [data]).map(
      (d: Record<string, unknown>, i: number) => {
        ...
        return {
          id: `aq-${i}`,
          name: ITALIAN_CITIES[i]?.name ?? `Station ${i}`,
