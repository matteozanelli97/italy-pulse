...
const SEV_COLORS: Record<string, string> = { low: '#00bcd4', medium: '#f59e0b', high: '#ef4444' };
    ...
    <div className="space-y-2">
      {alerts.map((a) => (
        <div key={a.id} className="flex items-start gap-2 py-0.5">
          <span className="text-[13px] flex-shrink-0">{TYPE_ICONS[a.type] || '📋'}</span>
          <div className="min-w-0 flex-1">
            <p className="truncate text-[11px] font-medium" style={{ color: SEV_COLORS[a.severity] || 'var(--text-secondary)' }}>{a.title}</p>
            <p className="text-[10px] truncate" style={{ color: 'var(--text-dim)' }}>{a.description}</p>
            {a.route && <p className="text-[9px] font-mono" style={{ color: 'var(--text-muted)' }}>{a.route}</p>}
          </div>
