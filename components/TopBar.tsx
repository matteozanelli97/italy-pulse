...
export default function TopBar() {
  ...
  return (
    <header className="flex items-center justify-between border-b px-4 py-1.5 flex-shrink-0 bg-dot-pattern"
      style={{ background: '#050505', borderColor: 'var(--border-dim)', minHeight: 44 }}>
      ...
      <div className="hidden md:flex items-center gap-4">
        ...
        <StatusPill icon={<NodeIcon />} label="SOURCES" value={`${activeSources}/7 ACTIVE`} />
        <StatusPill icon={<PingIcon />} label="NET" value="32ms" />
        <StatusPill icon={<DataIcon />} label="DATA" value={`${totalDataPoints} PTS`} />
        {aiSummary && (
          <div className="max-w-[280px] rounded px-2.5 py-1 text-[9px] leading-relaxed truncate"
...
function StatusPill({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="flex items-center gap-1.5 rounded-md px-2.5 py-1" style={{ background: 'rgba(59,130,246,0.04)', border: '1px solid var(--border-dim)' }}>
      <span className="flex h-4 w-4 items-center justify-center" style={{ color: 'var(--blue-500)' }}>{icon}</span>
      <div>
        <div className="text-[7px] font-bold uppercase tracking-[0.15em]" style={{ color: 'var(--text-muted)' }}>{label}</div>
        <div className="font-mono text-[9px] font-semibold" style={{ color: 'var(--text-secondary)' }}>{value}</div>
      </div>
    </div>
  );
}
...
function LockIcon() {
  return <svg viewBox="0 0 16 16" fill="none" className="h-3 w-3"><rect x="3" y="7" width="10" height="7" rx="1.5" stroke="currentColor" strokeWidth="1.3" /><path d="M5 7V5a3 3 0 016 0v2" stroke="currentColor" strokeWidth="1.3" /></svg>;
}
...
function PingIcon() {
  return <svg viewBox="0 0 16 16" fill="none" className="h-3 w-3"><path d="M1 12h2l2-4 2 8 2-10 2 6h4" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" /></svg>;
}

function DataIcon() {
  return <svg viewBox="0 0 16 16" fill="none" className="h-3 w-3"><rect x="2" y="8" width="3" height="6" rx="0.5" stroke="currentColor" strokeWidth="1" /><rect x="6.5" y="5" width="3" height="9" rx="0.5" stroke="currentColor" strokeWidth="1" /><rect x="11" y="2" width="3" height="12" rx="0.5" stroke="currentColor" strokeWidth="1" /></svg>;
}
