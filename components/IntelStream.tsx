...
const SOURCE_COLORS: Record<string, string> = {
  ANSA: '#2563eb', Repubblica: '#dc2626', Corriere: '#1e40af',
    ...
    <div className="flex flex-1 flex-col overflow-hidden" style={{ background: 'var(--bg-deep)' }}>
      ...
      <div className="border-b px-4 py-3 flex-shrink-0" style={{ borderColor: 'var(--border-dim)', background: 'var(--bg-panel)' }}>
        <div className="flex items-center gap-2 mb-2.5">
          <span className="flex h-6 w-6 items-center justify-center rounded" style={{ background: 'rgba(59,130,246,0.08)' }}>
            <svg viewBox="0 0 16 16" fill="none" className="h-3.5 w-3.5">
              <path d="M2 3h12M2 7h8M2 11h12M2 15h6" stroke="var(--blue-500)" strokeWidth="1.5" strokeLinecap="round" />
            </svg>
          </span>
          <h2 className="text-[11px] font-bold uppercase tracking-[0.15em]" style={{ color: 'var(--text-primary)' }}>
            Intel Stream
          </h2>
          <div className="flex-1" />
          {breakingCount > 0 && (
            <span className="breaking-indicator rounded-full px-2 py-0.5 text-[8px] font-bold"
        ...
        <div className="flex items-center gap-2 rounded-lg px-2.5 py-1.5"
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Filtra intelligence..."
            className="flex-1 bg-transparent text-[10px] outline-none placeholder:text-[var(--text-muted)]" style={{ color: 'var(--text-secondary)' }} />
          {search && <button onClick={() => setSearch('')} className="text-[11px] hover:text-white transition-colors" style={{ color: 'var(--text-dim)' }}>×</button>}
        </div>
      </div>
      ...
      <div className="flex items-center gap-1 overflow-x-auto border-b px-3 py-2 flex-shrink-0" style={{ borderColor: 'var(--border-dim)' }}>
        {CATEGORIES.map((cat) => {
          const catColor = CATEGORY_COLORS[cat.id] || 'var(--blue-500)';
          const isActive = activeTab === cat.id;
          return (
            <button key={cat.id} onClick={() => setActiveTab(cat.id)}
              className="flex-shrink-0 rounded-md px-2 py-1 text-[8px] font-semibold uppercase tracking-wider transition-all"
              style={{
                background: isActive ? (cat.id === 'all' ? 'rgba(59,130,246,0.10)' : `${catColor}15`) : 'transparent',
                color: isActive ? (cat.id === 'all' ? 'var(--blue-400)' : catColor) : 'var(--text-dim)',
                border: `1px solid ${isActive ? (cat.id === 'all' ? 'rgba(59,130,246,0.20)' : `${catColor}30`) : 'transparent'}`,
              }}>
              {cat.label}
            </button>
          );
      ...
      <div className="flex-1 overflow-y-auto">
        {loading && items.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-12">
            <span className="text-[10px]" style={{ color: 'var(--text-dim)' }}>Nessun risultato</span>
          </div>
        ) : (
          <AnimatePresence mode="popLayout">
            <div>
              {filtered.slice(0, 50).map((item, i) => {
                const catColor = CATEGORY_COLORS[item.category] || 'var(--blue-500)';
                return (
                  <motion.a
                    key={item.id}
                    href={item.url}
                    ...
                    className="group block px-4 py-2.5 transition-colors hover:bg-[var(--bg-hover)] border-b"
                    style={{ borderColor: 'var(--border-dim)' }}
                  >
                    {/* Source + category + time row */}
                    <div className="flex items-center gap-2 mb-1">
                      <SourceTag source={item.source} />
                      {/* Show category tag ONLY in "All" tab */}
                      {activeTab === 'all' && (
                        <span className="rounded px-1.5 py-0 text-[7px] font-bold uppercase"
                    ...
                    {/* Title */}
                    <p className="line-clamp-2 text-[10px] leading-relaxed font-medium group-hover:text-white transition-colors"
                      style={{ color: item.isBreaking ? 'var(--text-primary)' : 'var(--text-secondary)' }}>
                      {item.isBreaking && <span className="inline-block mr-1 text-[7px] font-bold px-1 rounded breaking-indicator" style={{ background: 'rgba(220,38,38,0.12)', color: '#dc2626' }}>URGENTE</span>}
                      {item.title}
                    </p>
                    {item.description && (
                      <p className="mt-0.5 line-clamp-1 text-[9px]" style={{ color: 'var(--text-dim)' }}>
                        {item.description}
                      </p>
                    )}
                  </motion.a>
  ...
  return (
    <span className="source-tag" style={{ color, borderColor: `${color}25`, background: `${color}08` }}>
      {favicon ? (
        ...
        <img src={favicon} alt="" className="h-2.5 w-2.5 rounded-sm" style={{ imageRendering: 'auto' }}
